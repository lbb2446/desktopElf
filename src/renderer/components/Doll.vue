<template>
  <div class="hello">
    ...
    <li class="doll"
      ...
      <img v-if="checked.includes(k)" :src="getImg(v)"
        :alt="i">
    </li>
<List header="Header" class="tool"
      footer="Footer"
      border>
      <CheckboxGroup v-model="checked">
        
        <ListItem v-for="(v,k) in config" :key="k">
...
import {face_b,face_f,bottom,acce,outer_b,outer_f,hair_f,hair_b,hair_s,top,under,} from './doll.js'

function seletedDecoration(arr){
  return arr.map((t)=>{return {value:t,label:t}})
}
...
export default {
  ...
  data() {
    return {
      ...
      checked: ["cat","hair_f","face_f","face_b","outside_f","top","bottom","under","body","outside_b","hair_b","hair_s"],
      body: {
        cat: "acce_glass_red",
        hair_f: "hair_normal_front_main_normal",
        face_f: "face_shy_front",
...
@keyframes frame {
    ...
    100% {background-position:  -9248px 0;}
}
.doll {
  position: absolute;
  width: 208px;
